...
#### Get Download Token
```
GET /api/storage/signed-get?key=path/to/file.jpg
```
...
#### List Objects
```
GET /api/storage/list-prefix?prefix=folder/
```
...
#### Get Object Info
```
GET /api/storage/object-info?key=path/to/file.jpg
```

#### Delete Object
```
DELETE /api/storage/object?key=path/to/file.jpg
```
...
- **Token-based Authentication**: Secure signed tokens for upload/download operations
