                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-medium">{formData.currency} {(item.line_total || 0).toFixed(2)}</span>
                          {isEditable && (
                            <Button 
                              type="button" 
                              variant="ghost" 
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="space-y-2 max-w-sm ml-auto">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{formData.currency} {formData.subtotal.toFixed(2)}</span>
              </div>
              {formData.tax_amount > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-600">VAT ({(formData.tax_rate * 100).toFixed(1)}%):</span>
                  <span className="font-semibold">{formData.currency} {formData.tax_amount.toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between border-t pt-2">
                <span className="font-bold">Total:</span>
                <span className="font-bold text-amber-600">{formData.currency} {formData.total_amount.toFixed(2)}</span>
              </div>
            </div>
          </div>
