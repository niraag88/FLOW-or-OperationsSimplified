...
export default function CreateFromExistingDialog({ open, onClose, onDocumentSelected }) {
  ...
  const [fullDo, setFullDo] = useState(null);
  ...
  const handleQuotationSubmit = async () => {
    ...
    const selectedDocument = enrichedQuotations.find(q => String(q.id) === String(selectedQuotationId));
    if (!selectedDocument) {
  ...
  const handleDoContinue = async () => {
    ...
    try {
      ...
      setFullDo(doData);
      setDoItems((doData.items || []).map(item => ({
        ...item,
  ...
  const handleDoSubmit = () => {
    const includedItems = doItems.filter(item => item.invoiceQty > 0);
    ...
    const adjustedItems = includedItems.map(item => ({
      ...item,
      quantity: item.invoiceQty,
      line_total: parseFloat(item.unit_price || 0) * item.invoiceQty,
    }));
    const subtotal = adjustedItems.reduce((sum, item) => sum + item.line_total, 0);
  ...
  const adjustedSubtotal = doItems
    ...
    .reduce((sum, item) => sum + parseFloat(item.unit_price || 0) * (item.invoiceQty || 0), 0);
  const taxRate = fullDo?.tax_rate ?? 0;
  ...
  const selectedDoInfo = enrichedDeliveryOrders.find(d => String(d.id) === String(selectedDeliveryOrderId));
          ...
          <>
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-500">
                {doItems.filter(i => i.invoiceQty > 0).length} of {doItems.length} items will be invoiced.
              </p>
            </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={doItems.filter(i => i.invoiceQty > 0).length === 0}
                data-testid="button-create-invoice"
              >
                Create Invoice
              </Button>
