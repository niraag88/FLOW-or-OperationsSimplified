...
export default function InvoicesListPrintView() {
  ...
  useEffect(() => {
    ...
    ['search', 'status', 'customerId', 'paymentStatus', 'dateFrom', 'dateTo', 'excludeYears'].forEach((key: any) => {
  ...
  const totSubtotal = invoices.reduce((s: any, inv: any) => s + (parseFloat(inv.subtotal) || 0), 0);
  const totVat      = invoices.reduce((s: any, inv: any) => s + (parseFloat(inv.vatAmount) || 0), 0);
  const totGrand    = invoices.reduce((s: any, inv: any) => s + (parseFloat(inv.amount) || 0), 0);

  const urlParams = new URLSearchParams(window.location.search);
  const filterParts: any[] = [];
  if (urlParams.get('search'))        filterParts.push(`Search: "${urlParams.get('search')}"`);
  if (urlParams.get('status'))        filterParts.push(`Status: ${urlParams.get('status')!.split(',').map((s: any) => s.charAt(0).toUpperCase() + s.slice(1)).join(', ')}`);
