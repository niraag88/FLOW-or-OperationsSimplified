...
export default function QuotationsListPrintView() {
  ...
  useEffect(() => {
    ...
    ['search', 'status', 'customerId', 'dateFrom', 'dateTo', 'excludeYears'].forEach((key: any) => {
  ...
  const totSubtotal = quotations.reduce((s: any, q: any) => s + (parseFloat(q.totalAmount) || 0), 0);
  const totVat      = quotations.reduce((s: any, q: any) => s + (parseFloat(q.vatAmount) || 0), 0);
  const totGrand    = quotations.reduce((s: any, q: any) => s + (parseFloat(q.grandTotal) || 0), 0);
