...
export default function PoGrnReport({ purchaseOrders, goodsReceipts, suppliers = [], companySettings, canExport }: PoGrnReportProps) {
  ...
  const [poDateRange, setPoDateRange] = useState<any>("all");
  ...
  const [grnSelectedSuppliers, setGrnSelectedSuppliers] = useState<any[]>([]);
  const [grnDateRange, setGrnDateRange] = useState<any>("all");
  ...
  const formatDate = (dateString: any) => {
    ...
    try {
      ...
      return date && !isNaN(date.getTime()) ? format(date, 'dd/MM/yy') : '-';
    } catch {
      return '-';
    }
  };
  ...
  const uniqueStatuses = [...new Set(purchaseOrders.map((po: any) => po.status).filter(Boolean))].sort();
  ...
  const filteredGRNs = useMemo(() => {
    const activeGRNs = (goodsReceipts || []).filter((grn: any) => grn.status !== 'cancelled');
    return activeGRNs.filter((grn: any) => {
  ...
  const totals = useMemo(() => {
    return filteredPOs.reduce((acc: any, po: any) => {
        ...
        <CardContent className="space-y-4">
                    ...
                    {uniqueStatuses.map((status: any) => (
                      <div key={status} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            setPoSelectedStatuses(prev => checked ? [...prev, status] : prev.filter((s: any) => s !== status));
                            setCurrentPage(1);
                          }}
                        />
          ...
          {(poSelectedStatuses.length > 0 || poSelectedSuppliers.length > 0) && (
            <div className="flex flex-wrap gap-2">
              {poSelectedStatuses.map((status: any) => (
                <Badge key={status} variant="secondary" className="gap-1">
                  Status: {status}
                  <X className="h-3 w-3 cursor-pointer" onClick={() => { setPoSelectedStatuses(prev => prev.filter((s: any) => s !== status)); setCurrentPage(1); }} />
                </Badge>
              ))}
              {poSelectedSuppliers.map((supplierId: any) => {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {grnSelectedSuppliers.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {grnSelectedSuppliers.map((supplierId: any) => {
                const supplier = suppliers.find((s: any) => s.id === supplierId);
                return (
                  <Badge key={supplierId} variant="secondary" className="gap-1">
                    Supplier: {supplier?.name}
