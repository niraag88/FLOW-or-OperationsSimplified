...
export default function StockOnHandReport({ products }) {
  ...
  const brands = useMemo(() => {
    const set = new Set(activeProducts.map(p => p.brandName).filter(Boolean));
    return [...set].sort();
  }, [activeProducts]);

  const summary = useMemo(() => ({
    total: activeProducts.length,
    inStock: activeProducts.filter(p => (p.stockQuantity || 0) > LOW_STOCK_THRESHOLD).length,
    lowStock: activeProducts.filter(p => (p.stockQuantity || 0) > 0 && (p.stockQuantity || 0) <= LOW_STOCK_THRESHOLD).length,
    outOfStock: activeProducts.filter(p => (p.stockQuantity || 0) === 0).length,
  }), [activeProducts]);
