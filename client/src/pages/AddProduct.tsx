...
export default function AddProduct() {
  ...
  const [brands, setBrands] = useState<any[]>([]); // Added state for brands
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        setBrands(brandsData.filter((b: any) => b.isActive)); // Fetch and filter active brands
        if (settingsList.length > 0) {
          setFxRate((settingsList[0] as any).fx_gbp_to_aed || 4.85);
        }
      } catch (error: any) {
  ...
  const handleSave = async (mode: any) => {
    setLoading(true);
    if (!(await validate())) {
      setLoading(false);
      return;
