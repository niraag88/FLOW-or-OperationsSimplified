...
export default function StockCountNew() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const itemsWithStock = items.filter((item: any) => quantities[item.product_id] !== undefined);
  ...
  const totalQuantity = Object.values(quantities).reduce((sum: any, qty: any) => sum + (qty || 0), 0);
