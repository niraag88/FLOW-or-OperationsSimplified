...
- **Explicit Staff Role Matrix (Task #370, RF-7B)**: Write routes carry an explicit role list at the `requireAuth([...])` site instead of relying on the default `requireAuth()` (which silently allowed Staff). Source of truth for the matrix:
  - **Admin / Manager only**: Brands POST/PUT/DELETE; Suppliers POST/PUT/DELETE; Products POST/PUT/DELETE/`/bulk`/`/:id/adjust-stock`; Inventory `POST /api/stock-movements/bulk`; Stock-counts POST + DELETE; Customers DELETE; Delivery Orders DELETE + `PATCH /:id/cancel`; Invoices DELETE + `POST /:id/process-sale`; Quotations PUT + DELETE.
  - **Admin / Manager / Staff (operational)**: Customers POST + PUT (so Staff can capture walk-in customers from the invoice/DO/quotation forms); Invoices POST + PUT + `from-quotation` + `PATCH /:id/cancel` + `PATCH /:id/scan-key` + `DELETE /:id/scan-key` + `PATCH /:id/payment`; Delivery Orders POST + PUT + `PATCH /:id/scan-key` + `DELETE /:id/scan-key`; Quotations POST + `PATCH /:id/convert`.
