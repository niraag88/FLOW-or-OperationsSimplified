...
- **Real File-Bytes Backups + Year Sealing (Task #427)**: Backups now bundle every uploaded scan + logo into a real `.tar.gz` archive (`backups/files/files-YYYYMMDD-HHMMSS.tar.gz`) alongside the SQL dump — replacing the legacy "manifest of filenames" approach. Implementation: `scripts/archiveFiles.js` streams bucket → tar → gzip → object storage (peak heap = one 5MB file) and excludes (a) the `backups/` prefix and (b) any `<scope>/<closedYear>/` paths. `scripts/restoreFiles.js` is the inverse. New `backup_runs.files_*` columns persist the per-run archive metadata (`server/ensureSchema.ts` adds them as a belt-and-braces alongside `npm run db:push`). When an accounting year is closed (`PUT /api/books/:id` Open→Closed), `server/sealYearArchive.ts` runs synchronously inside the route, building a permanent year-only archive at `backups/years/year-<year>.tar.gz` and upserting `ops.year_archives` (catalog lives in ops schema so it survives `DROP SCHEMA public CASCADE` on restore in lockstep with the storage object). Re-closing a previously-reopened year overwrites the seal. Cloud restore (`POST /api/ops/backup-runs/:id/restore`) downloads the files tarball pre-lock, runs DB restore + reconcile, then calls `server/restoreRollingFiles.ts` inside the same destructive lock — which snapshots the rolling-file set to `backups/_pre_restore/<runId>/`, wipes it, streams the archive back, and on success deletes the snapshot (failure keeps it for inspection and auto-rolls back). New `POST /api/ops/restore-files-upload` endpoint mirrors this for uploaded `.tar.gz` (RESTORE_PHRASE typed confirmation, 500 MB cap). UI (`BackupSettings.tsx`) shows two download buttons per backup row (DB / Files), a "Sealed Year Archives" panel with per-year download, and an "Upload & Restore Files" button. Closed-year scans are NEVER touched by restores or rolling backups — they live exclusively in their year-archive.
