...
test.describe('Phase 11 — Cleanup', () => {
  ...
  test('step 75: generate AUDIT_REPORT.md from run state and Playwright JSON report if available', async () => {
    ...
    const report = [
      ...
      '| 5 | 05-quotations | QT list renders → New QT form (8 items browser) → QT-01 submit via browser detail page → QT-02 cancelled via browser "Cancel Quotation" menu item in actions dropdown → cancelled QT shows "cancelled" badge → export download → QT print views ×2 | QT statuses, 8+12 items confirmed |',
      '| 6 | 06-invoices | Invoice list renders → INV-01 (6 items browser form) → INV-01 submit via browser → INV-04 (3 items browser form) → INV-04 cancel via browser actions dropdown (Cancel Invoice → Yes Cancel) → PAID/OUTSTANDING badges → invoice print views ×2 → payments ledger | Lifecycle: deliver/pay via API (payment forms are complex sub-widgets); cancel confirmed via browser |',
      '| 7 | 07-delivery-orders | DO list renders → DO-01 created from INV-01 via "Create from Existing" browser flow → DO-02 (3 items browser form) → DO-01 delivered via browser → DO-02 cancelled via browser actions dropdown → DO print view → export download | DO status propagation to invoice; DO-03 API-assisted (browser budget consumed by DO-01/DO-02) |',
      '| 8 | 08-inventory | /Inventory stock quantities visible → stock > 0 products (post-GRN) → stock count form → stock movements tab → inventory export CSV/PDF download attempt | CSV/PDF export download events |',
